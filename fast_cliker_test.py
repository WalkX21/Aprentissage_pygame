...
class Area():
    ...
    def outline(self,frame_color,thickness):# outline of an existing rectangle
      outline_rect = pygame.Rect(
        ...
        self.rect.width + thickness * 2,
        self.rect.height + thickness * 2,
      )
      pygame.draw.rect(mw,frame_color,outline_rect,thickness)
...
time_text.set_text('Time:', 30, DARK_BLUE)
...
score_text.set_text('Count:', 35, DARK_BLUE)
...
while True:
    ...
    if int(new_time) - int(
    cur_time
  ) == 1:  #check if there is a difference of 1 second between the old and new time
    ...
    if points >= 5:
        ...
        resul_time.set_text(
            "Completion time: " + str(int(new_time - start_time)) + " sec", 40, #pk new time - start time (resonement)
            DARK_BLUE)
